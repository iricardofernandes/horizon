ALTER TABLE fiscal_tax_rules ADD COLUMN purpose text NOT NULL DEFAULT 'normal'
  CHECK (purpose IN ('normal', 'return', 'complementary', 'adjustment'));
...
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended(
    ...
    NEW.issuer_establishment_id || ':' || NEW.issuer_regime || ':' || NEW.recipient_regime || ':' ||
    NEW.origin_state || ':' || NEW.destination_state || ':' || NEW.subject_kind || ':' ||
    NEW.subject_id || ':' || NEW.classification_kind || ':' || NEW.classification_code,
    0
  ));
  IF EXISTS (
