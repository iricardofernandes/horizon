 ...
 * Response bodies can contain credentials, so only authenticated ciphertext is
 * stored. Opted-in requests fail closed on Redis failure. A pending claim remains
 * after a failed completion write: repeating an already committed effect is unsafe.
 */
export class IdempotencyStore {
  ...
  /** Keyed identities keep passwords and opaque credentials out of Redis key names. */
  principalFingerprint(identity: unknown): string {
    return this.digest('principal', canonicalJson(identity))
  }

  async begin(request: IdempotencyRequest): Promise<IdempotencyResult> {
    const storageKey = `identity:idempotency:${this.digest(
      'scope',
