import { type Either, left, right } from '@/core/either'
import { ValueObject } from '@/core/entities/value-object'
import { InvalidInputError } from '@/core/errors/errors/invalid-input-error'

interface PersonNameProps {
  readonly value: string
}

/** A human's name. Personal data: encrypted at rest, never indexed (ADR 0026). */
export class PersonName extends ValueObject<PersonNameProps> {
  static create(raw: string, field = '/name'): Either<InvalidInputError, PersonName> {
    const collapsed = raw.trim().replace(/\s+/g, ' ')

    if (collapsed.length === 0) return left(new InvalidInputError(field, 'name is required'))
    if (collapsed.length > 200)
      return left(new InvalidInputError(field, 'name must be at most 200 characters'))

    return right(new PersonName({ value: collapsed }))
  }

  get value(): string {
    return this.props.value
  }

  protected componentsOf(): readonly unknown[] {
    return [this.props.value]
  }
}
