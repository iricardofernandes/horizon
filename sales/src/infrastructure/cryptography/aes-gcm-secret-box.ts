...
const CONTEXT = Buffer.from('horizon:sales:customer:v1', 'utf8')
...
export class AesGcmSecretBox extends SecretBox {
  ...
  open(secret: string, sealed: string): string | null {
    try {
      ...
      const [salt, nonce, ciphertext, tag] = parts
        .slice(1)
        .map((part) => Buffer.from(part, 'base64url'))
      if (salt?.length !== 16 || nonce?.length !== 12 || tag?.length !== 16 || !ciphertext)
        return null
